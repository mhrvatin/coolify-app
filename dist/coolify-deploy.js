...
export async function findExistingAppUuid(config) {
    const apps = await coolifyRequest(clientConfig(config), '/applications');
    const match = apps.find((app) => app.name === config.name &&
        (app.project_uuid === undefined || app.project_uuid === config.projectUuid));
    return match?.uuid;
}
async function findOrCreateAppUuid(config) {
    const existing = await findExistingAppUuid(config);
    if (existing)
        return existing;
    const created = await coolifyRequest(clientConfig(config), '/applications/private-deploy-key', { method: 'POST', body: JSON.stringify(buildCreatePayload(config)) });
    return created.uuid;
}
...
//# sourceMappingURL=coolify-deploy.js.map